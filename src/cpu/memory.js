...
import * as PPU from '../ppu/registers';
...
export default class Memory {
  ...
  r8(addr) {
    ...
    switch (addr >> 12) {
      ...
      default:
        if (addr == PPU.OAMDMA) this.nes.ppu.r8(addr);
        else if (addr == 0x4016) return this.nes.controller.read();
        else if (addr < 0x4018) return this.apu[addr & 0x1f];
        else if (addr < 0x4020) return 0;
