...
export default class Cpu {
  ...
  pageCrossedCycles(addr) {
    const {branchCycles} = this.decodeInstruction();
    return (this.pc & 0xff00) != (addr & 0xff00) ? branchCycles : 0;
  }

  runCycle() {
    const {opcode, execute, mode, bytes, cycles} = this.decodeInstruction();
    const next = (this.pc + 1) & 0xffff;
