...
module.exports = class Cpu {
  ...
  push(val) {
    Debug('push');
    this.mmu.writeByte(val, this.sp++);
  }
  ...
  runCycle() {
    ...
    switch (Instructions.mode[opcode]) {
      case MODE_IMM:
        src = this.mmu.readByte(next);
        break;
      case MODE_ABS: {
        const addr = this.mmu.readWord(next);
        src = this.mmu.readByte(addr);
        store = (val) => this.mmu.writeByte(val, addr);
        break;
      }
      case MODE_ZERO_PAGE: {
        const addr = this.mmu.readByte(next);
        src = this.mmu.readByte(addr);
        store = (val) => this.mmu.writeByte(val, addr);
        break;
      }
      ...
      case MODE_ACC:
        ...
        break;
      case MODE_ABS_X: {
        const addr = this.mmu.readWord(next) + this.x;
        src = this.mmu.readByte(addr);
        store = (val) => this.mmu.writeByte(val, addr);
        break;
      }
      case MODE_ABS_Y: {
        const addr = this.mmu.readWord(next) + this.y;
        src = this.mmu.readByte(addr);
        store = (val) => this.mmu.writeByte(val, addr);
        break;
      }
      case MODE_ZERO_PAGE_X: {
        const addr = this.mmu.readByte(next) + this.x;
        src = this.mmu.readByte(addr);
        store = (val) => this.mmu.writeByte(val, addr);
        break;
      }
      case MODE_ZERO_PAGE_Y: {
        const addr = this.mmu.readByte(next) + this.y;
        src = this.mmu.readByte(addr);
        store = (val) => this.mmu.writeByte(val, addr);
        break;
      }
      case MODE_IND:
        src = this.mmu.readWord(this.mmu.readWord(next));
        break;
      case MODE_IDX_IND: {
        const addr = this.mmu.readWord(this.mmu.readByte(next) + this.x);
        src = this.mmu.readByte(addr);
        store = (val) => this.mmu.writeWord(val, addr);
        break;
      }
      case MODE_IND_IDX: {
        const addr = this.mmu.readWord(this.mmu.readByte(next)) + this.y;
        src = this.mmu.readByte(addr);
        store = (val) => this.mmu.writeWord(val, addr);
        break;
      }
      case MODE_REL: {
        const byte = this.mmu.readByte(next);
